...
\end{itemize}
Questa casistica risulta di maggiore interesse poiché:
\begin{itemize}
...
\item valutazione dei risultati utilizzando l'algoritmo senza e con l'estensione introdotta nella sezione \ref{estensioneAlg}.
\end{itemize}
...
\begin{itemize}
\item solamente l'asset \textit{Child 2} perde i permessi che potrebbe mantenere in ambiente collaborativo, ovvero \textit{anonymize};
...
\end{itemize}
Questa casistica risulta però più limitata da un punto di vista della presentazione dei risultati raggiunti: non vi è un cambiamento di ambiente e la gerarchia può essere espressa mediante un albero. Infine per MOSAICrOWN risulta interessante solamente l'utilizzo dell'algoritmo esteso: cambiamenti portati da un owner non devono avere effetti su dati sui quali non può definire regole.
\begin{figure}[H]
\centering
...
\begin{itemize}
\item arricchendo la sintassi ODRL inserendo termini relativi al processo di merging. In questo modo si seguono i principi dei \textit{Linked Data} poiché da una policy è possibile raggiungere i vari documenti che hanno contribuito alla sua creazione;
...
\end{itemize}
Alla luce di tali problematiche per riuscire ad utilizzare questi costrutti all'interno di un mercato digitale risulta necessario discretizzare il \textit{range} utilizzabile nei vincoli. Il procedimento proposto andrebbe a ridurre la flessibilità del linguaggio ma, allo stesso tempo, permetterebbe di attuare logiche di merging anche su questa porzione di ODRL.  
\chapter{Conclusione}
